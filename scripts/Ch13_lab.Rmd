...
flights %>%
  ...
  summarise(combos = n_distinct(tailnum, carrier)) %>% #this is the problem.  n_distinct is essentially counting the same thing that was grouped by in the line above.  therefor it will always be 1
  filter(combos > 1)

# this also doesn't work - why doesn't n_distinct work here?
flights %>%
  filter(!is.na(tailnum)) %>%
  summarise(combos = n_distinct(tailnum, carrier)) %>%
  filter(combos >1)
